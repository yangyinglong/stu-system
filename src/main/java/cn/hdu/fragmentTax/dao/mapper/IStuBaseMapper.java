...
public interface IStuBaseMapper {
    ...
    @Select("SELECT `id`, `stu_id`, `name`, `phone`, `sex`, `u_school`, `u_major`, `counsellor_id`, `tutor_id`, `state`, `created_time`, `changed_time`, `id_card`, `master_major`, `political_outlook`, `biogenic_land`, `master_direction`, `e_mail`, `emergency_contact`, `emergency_phone`, `secretary` FROM `stu_base` WHERE `tutor_id` = #{tutorId} and `state` = 1 limit #{page}, 10")
    @Results({
            @Result(property = "id", column = "id"),
            @Result(property = "stuId", column = "stu_id"),
            @Result(property = "name", column = "name"),
    ...
    List<StuBaseEntity> queryByTutorIdWithPage(@Param("tutorId") String tutorId, @Param("page") int page);
    ...
    @Select("SELECT `id`, `stu_id`, `name`, `phone`, `sex`, `u_school`, `u_major`, `counsellor_id`, `tutor_id`, `state`, `created_time`, `changed_time`, `id_card`, `master_major`, `political_outlook`, `biogenic_land`, `master_direction`, `e_mail`, `emergency_contact`, `emergency_phone`, `secretary` FROM `stu_base` WHERE `stu_id` like #{stuId} and `name` like #{stuName} and `state` = 1 order by `stu_id` desc limit #{page} , 10")
    @Results({
            @Result(property = "id", column = "id"),
            @Result(property = "stuId", column = "stu_id"),
            @Result(property = "name", column = "name"),
    ...
    List<StuBaseEntity> queryForCounerWithPage(@Param("stuId") String stuId, @Param("stuName") String stuName, @Param("page") int page);


    @Select("SELECT count(*) FROM `stu_base` WHERE  `state` = 1 and `stu_id` like #{stuId} and `name` like #{stuName} ")
    @Results({
            @Result(property = "num", column = "count(*)")
    })
    int queryNumForCouner(@Param("stuId") String stuId, @Param("stuName") String stuName);

    @Select("SELECT count(*) FROM `stu_base` WHERE  `state` = 1 and `tutor_id` = #{tutorId}")
    @Results({
            @Result(property = "num", column = "count(*)")
    })
    int queryNumByTutorId(@Param("tutorId") String tutorId);
}